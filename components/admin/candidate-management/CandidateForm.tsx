...
import { Candidate, CreateCandidateInput, UpdateCandidateInput, CandidateCredentials } from '@/lib/types/candidate';
...
export function CandidateForm({ candidate, onSubmit, onCancel }: CandidateFormProps) {
  ...
  const generatePassword = () => {
    const password = candidateService.generatePassword();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        ...
        candidateService.updateCandidate(candidate!.id, updateData);
        onSubmit();
      } else {
        // Create new candidate - return credentials
        const result = candidateService.createCandidate(formData);
        onSubmit(result.credentials);
      }
    } catch (error) {
    ...
    <form onSubmit={handleSubmit}>
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {/* Username - only for new candidates */}
          {!isEditing && (
